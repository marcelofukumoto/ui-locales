...
- ❌ `cluster.machineConfig.linode.typeLabel` block scalar double-content bug (introduced in run 5) — parse error at line 1993 — needs fix in run 6
...
- Previous improve runs: run 1 → 57.5%, run 2 → 86%, run 3 → 91.1%, run 4 → ~91%+, run 5 → 89.3% (fixed nav.support + ~20 new strings, but broke linode.typeLabel)
